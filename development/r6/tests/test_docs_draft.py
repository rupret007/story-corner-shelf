...
class R6DocumentationDraftTests(unittest.TestCase):
    ...
    def test_a1mini_sunlu_petg_workflow_is_explicit_and_qualification_only(self) -> None:
        readme = " ".join(self.documents["README.md"].split())
        gate_card = " ".join(self.documents["PRINT_ME_FIRST.md"].split())
        combined = f"{readme} {gate_card}".lower()
        for phrase in (
            "bambu lab a1 mini",
            "0.4 mm nozzle",
            "textured pei plate",
            "sunlu standard black petg 1.75 mm",
            "b0d1kc72yp",
            "current studio pla profile must not be reused",
            "sunlu petg @bbl a1m 0.4 nozzle",
            "250 °c first layer",
            "245 °c other layers",
            "60 °c first and other layers",
            "flow ratio",
            "0.94",
            "9 mm³/s",
            "0.20 mm strength",
            ...
            "grid",
            "5 mm baseline",
            "off by system default",
            "10% minimum, 30% maximum, 90% for overhangs",
            "60–65 °c for 6 hours",
            "manual coarse and fine flow rate",
            "100% scale on x, y, and z",
            "one plate at a time",
            "35 °c or below",
            "zero nominal bed-axis margin",
            "build_bambu_qualification_projects.py --output ../story-corner-r6-bambu-a1mini-sunlu-petg-qualification-v1",
            ".venv/bin/python build_bambu_qualification_projects.py --output ../story-corner-r6-bambu-a1mini-sunlu-petg-qualification-v1",
            "../story-corner-r6-bambu-a1mini-sunlu-petg-qualification-v1/",
            "local, unsliced qualification workspace with no g-code",
        ...
        self.assertIn("keep", gate_card.lower())
        self.assertIn("r6_dev_blocked_wall_screw_bearing_coupon_solid_no_hole", gate_card.lower())
        self.assertIn("do not print a wall-screw bearing coupon", gate_card.lower())
        self.assertIn("must not be published as a production artifact", combined)
