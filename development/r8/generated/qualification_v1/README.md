...
1. Start a new `Bambu Lab A1 mini 0.4 nozzle` project with the Textured PEI
...
3. Select `SUNLU PETG @BBL A1M 0.4 nozzle` and `0.20mm Strength @BBL A1M`.
   Explicitly verify: 0.20 mm layers, 6 wall loops, 5 top / 3 bottom shell
   layers, 25% **grid** infill, Support OFF, Brim type `Outer brim only`,
   5.0 mm brim width, and 0.1 mm brim-object gap.
4. Verify 250 C first layer / 245 C later layers, 60 C bed, 0.94 flow ratio,
   9 mm^3/s maximum volumetric speed, 10-30% normal fan, and 90% overhang fan.
   Dry PETG at 60-65 C for 6-8 hours and record the spool lot, drying cycle,
   and flow calibration before printing.
...
the plate** and a 45 degree bed yaw. Its part envelope is 163.4367 mm square by
152.4 mm high; brim plus the independent 2 mm-per-edge reserve requires
177.4367 mm square. Center it on the plate. If Studio reports an exclusion-zone
