...
## Gate 0 — corrected-pose 0.4 clearance qualification

### Articles and order

Use the two exact individual files from
...
A previously printed frozen R8 v2 receiver may be reused only because its
canonical geometry digest matches the v5 receiver. Do not print any legacy R8
identity-pose key; those saved poses contain a large handle cantilever.

### Method

1. Identify the corrected-pose key and record its v5 manifest identity.
2. Place the receiver on a stable table; do not hold it where hand pressure can
   flex the opening.
3. Before fit, inspect the key's two keyed-head wings. Reject on a loose strand,
   curl, torn perimeter, layer separation, or visible droop.
4. Insert the 0.4 key using the intended straight service motion. If it binds,
   needs a tool, visibly deforms the receiver, or causes whitening/cracking,
   stop.
5. Perform ten gentle full seat/release cycles
   and inspect after cycles 1, 5, and 10. Record any change in feel; do not add
   lubricant.

### Gate 0 result

The authored 0.4 interface passes only when the corrected key fully seats and
releases by hand
through ten gentle cycles without tools, destructive force, crack, whitening,
permanent set, lost engagement, or increasing bind. This Gate does not authorize
rescaling, full-shelf printing, wall work, or load.
...
## Gate 5 — qualification-v5 review and stop
...
The final v5 result may be called **qualification-article dry-fit pass** only
...
2. the emitted Palatine Moderne v3 one-bay ledger/front-beam/support/cassette
   fixture, still requiring physical fit review;
...
[MATERIALS_AND_HARDWARE.md](MATERIALS_AND_HARDWARE.md), target contents load,
exact CAD bore
...
| 4 — cable/bookend service |  |  |  | v5 review only if pass |
| 5 — qualification-v5 review |  |  |  | stop; no one-bay or wall work |
