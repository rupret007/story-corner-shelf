...
- [ ] The package is `r9_compact_bookend_petg_qualification_v5`.
...
### 7. Stop the qualification bundle; use the separate one-bay guide

Qualification v5 deliberately contains no one-bay pose. The separate,
versioned `generated/one_bay_prototype_v3` package now emits one exact 160 mm
Palatine Moderne tabletop bay with member seats, cassette locators, and
corrected printed wall-bore candidates. Use only that package's own README and
assembly guide for the five-part bay.

Do not create a substitute with these loose coupons, R8 cassettes, tape, glue,
fasteners, scaling, or guessed alignment. Qualification v5 ends after the dry
fits above. The one-bay v3 remains unloaded and does not authorize a full-L
shelf or wall installation.
...
The selected screw/washer fixture candidate and exact quantities are in
[MATERIALS_AND_HARDWARE.md](MATERIALS_AND_HARDWARE.md). They are not drilling
or installation authorization. This document intentionally contains no wall-
bore coordinates, installation torque, proof load, or rated load.
