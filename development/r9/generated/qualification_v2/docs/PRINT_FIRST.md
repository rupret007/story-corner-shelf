...
- long/through wall: approximately **60 in**;
- short/return wall: approximately **36 in**;
...
### Stage 0 — R8 clearance gate (print this first)

Use only the individual neutral 3MF files in:
...
4. `MODEL_ONLY_r8_clearance_key_0p3.3mf`
5. `MODEL_ONLY_r8_clearance_key_0p2.3mf`

The values are clearance **per face**. Let every part cool completely before
testing. Try the keys from loosest to tightest: 0.5, 0.4, 0.3, then 0.2 mm. The
authored R8/R9 interface baseline is 0.4 mm. The 0.4 key must seat and release
by hand without tools, cracking, whitening, permanent set, or destructive
force. The tighter keys are diagnostic; never force them.
...
```text
development/r9/generated/qualification_v2/
```

Its package identity is `r9_compact_bookend_petg_qualification_v2`. Before
...
Follow [ASSEMBLY.md](ASSEMBLY.md) on a padded table. Qualification v2 permits
...
- photos of the 0.5 and 0.4 R8 keys seated and removed;
