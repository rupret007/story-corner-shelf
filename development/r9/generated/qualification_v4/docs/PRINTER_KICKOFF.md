...
good,” or authorization for a previous plate is not sufficient. Each later
...
```text
development/r8/generated/qualification_v2/individual_model_only_3mf/MODEL_ONLY_r8_clearance_ladder_receiver.3mf
```

The file's SHA-256 in the frozen R8 v2 manifest is:

```text
8d11964f9f4ba58613f02fc24abb8950a6a0ecba2a5148187cf18e5a09a85d91
```

Use the individual neutral 3MF only. Do not open the combined R8 catalog, use
...
| Support | Off for the receiver and all four clearance keys |
...
cooled-part inspection may the clearance keys be prepared. Use one individual
neutral 3MF per first-article plate, in this exact loosest-to-tightest order:

1. `MODEL_ONLY_r8_clearance_key_0p5.3mf`
2. `MODEL_ONLY_r8_clearance_key_0p4.3mf`
3. `MODEL_ONLY_r8_clearance_key_0p3.3mf`
4. `MODEL_ONLY_r8_clearance_key_0p2.3mf`

They are in the same directory as the receiver. Preserve each saved pose at
100% scale with Support Off and repeat the complete Prepare, Preview,
authorization, first-layer, cooling, record, and inspection gates for every
file.

Test the cooled keys 0.5, 0.4, 0.3, then 0.2 mm per face. Never force a key.
The authored interface is 0.4 mm per face; that key must fully seat and release
by hand for ten gentle cycles without tools, cracking, whitening, permanent
set, lost engagement, or increasing bind. The 0.3 and 0.2 keys are diagnostic
only. If 0.4 fails, stop Stage 0, preserve the failed record, and correct the
